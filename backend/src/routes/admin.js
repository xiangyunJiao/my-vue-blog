...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = db
      .prepare(
        `SELECT p.id, p.title, p.slug, p.excerpt, p.status, p.published_at, p.created_at, p.updated_at,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const tx = db.transaction(() => {
      const info = db
        .prepare(
          `INSERT INTO posts (title, slug, excerpt, body_md, status, category_id, published_at, updated_at)
           VALUES (@title, @slug, @excerpt, @body_md, @status, @category_id, @published_at, datetime('now'))`
        )
        .run({
          title,
          slug,
